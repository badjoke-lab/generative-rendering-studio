...
Status: in progress. Browser-decodable MP4/WebM can be loaded as the main source; decoded frames feed the existing Original/Glyph/Point/Particle paths; play/pause/start/end/seek controls follow the video clock; and the currently displayed frame can be saved as PNG/WebP. Temporal-coherence hardening now includes deterministic source-space sampling and stable Symbols glyph identity so unchanged source positions do not get reassigned merely because adjacent samples enter or leave the active set. Browser evidence verifies a visible moving object across Original, Point and Glyph/Symbols output. Original and transformed video can also be shown in the same video-only composition with adjustable original opacity, and current-frame PNG/WebP export preserves that composition. Existing Stage 1 + Stage 2 Chromium/WebKit regression gates remain green. Remaining Stage 3 work is video texture/mask/analysis-input groundwork plus further coherence hardening where real footage exposes gaps, followed by the Public Alpha completion/release gate.
